...
각 케이스는 in-process gRPC(`InProcessServerBuilder`/`InProcessChannelBuilder`)로 실제 stub → service 직렬화·스트림 경로를 그대로 태우되 소켓은 거치지 않는다. 실행 방법과 최근 실행 결과는 [`docs/test-report.md`](test-report.md) 참고.
...
- **실제 두 프로세스 간 TCP/HTTP2 통신**: 자동화 테스트는 전부 in-process 채널이라 소켓을 거치지 않음. `docs/test-report.md`의 End-to-End 수동 검증 절차로 대체.
